...
export const TuViCalc = (function () {
    'use strict';
     ...
     */
    /**
     * Tính Tiểu Hạn (Vòng 12 năm)
     * Dựa vào Chi năm sinh để xác định cung khởi (tuổi 1)
     * Nam đi Thuận, Nữ đi Nghịch
     */
    function tinhTieuVan(chiNamSinh, gioiTinh, tuoi) {
        // Tam Hợp khởi tại các cung xung:
        ...
        let cungPos;
        if (gioiTinh === 'nam') {
            cungPos = (startPos + tuoi - 1) % 12;
        } else {
            cungPos = ((startPos - tuoi + 1) % 12 + 12) % 12;
        }
        return { cungPos, tuoi };
    }

    /**
     * Xác định Lưu Niên (Cung của năm xem)
     */
    function tinhLuuNien(chiNamXem) {
        return chiNamXem;
    }
    ...
    function calculate(params) {
        ...
        // 13. Tiểu Vận
        const tieuVan = tinhTieuVan(canChiNam.chiIndex, gioiTinh, tuoi);

        // 13b. Lưu Niên
        const luuNienPos = tinhLuuNien(canChiNamXem.chiIndex);
